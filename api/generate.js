...
async function selfVerifyAndCorrectBody({ client, model, body, requiredTechs = [], minLen, maxLen, tsukkomiName }) {
  const checklist = [
    ...
    "",
    "■最終チェックとしての再構成プロセス（60点→100点）",
    "- いまの本文を「一度は100点だと思って出力しようとしたが、実は60点だった漫才ネタ」とみなし、観客目線で弱い箇所・笑いが薄い箇所・意外性が足りない箇所を洗い出す。",
    "- フロントで指定された「採用する技法」が、それぞれボケ・ツッコミの具体的な掛け合いの中で十分に使われているかを再確認し、不足している技法があれば台詞や展開を追加・修正して必ず使用する。",
    "- ネタの題材や固有名詞、時事的な内容について、インターネット検索で最新の一般的な情報を確認したかのように検証し、明らかな事実誤認や古い情報があれば自然な形で修正する（検索手順そのものは出力に書かない）。",
    "- そのうえで、ボケ・ツッコミ・オチを必要な範囲で積極的に書き直し、『より意外性があり、面白く笑えて100点と言える完成版』になるように改稿する。",
    "",
