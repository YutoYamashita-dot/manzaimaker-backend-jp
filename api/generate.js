...
// 共通システム指示 (役割に「指定された題材を徹底的に主軸化すること」を追加)
const SYSTEM_INSTRUCTION = `あなたはプロの漫才作家です。指定された「題材」を100%主軸とし、最初から最後までその題材について徹底的にボケてツッコみあう、爆笑できる漫才台本を日本語で作成または修正してください。テーマと関係のない無駄な世間話などに脱線してはいけません。
...
あと約 ${diff}文字 ほど内容が不足しています。テーマである「${theme}」に関するストーリー展開や面白い議論は一切崩さず、ボケのやりとりを2〜3往復分追加して内容を肉付けし、必ず ${minLimit}文字以上 にしてください。`;
      } else {
        const diff = currentLen - targetLen;
        fixInstruction = `現在の文字数は ${currentLen}文字 で、目標（${minLimit}〜${maxLimit}文字 / 中央値 ${targetLen}文字）に対して多すぎます。
約 ${diff}文字 ほど長すぎます。テーマである「${theme}」を軸にした話の筋道やボケは変えないまま、冗長な表現を削り、必ず ${maxLimit}文字以下 にしてください。`;
      }
