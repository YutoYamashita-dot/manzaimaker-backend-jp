...
export default async function handler(req, res) {
 ...
 // 生成前：残高チェックのみ（消費なし）
    ...
    */
    ...
    // AIが生成した文字列に混入する可能性がある特殊なUnicode（行区切り文字など）、
    // 不正なサロゲートペア、制御文字（C0, C1, BOM等）を完全に除去・正規化し、
    // iOS/SwiftのJSONDecoderでのパースクラッシュやデコードエラーを防止します。
    const sanitizeStringForSwift = (str) => {
    ...
    const sanitizedBody = sanitizeStringForSwift(body);
    const sanitizedTitle = sanitizeStringForSwift(title);
    const sanitizedStructure = Array.isArray(structureMeta) ? structureMeta.map(sanitizeStringForSwift) : [];
    const sanitizedTechniques = Array.isArray(techniquesForMeta) ? techniquesForMeta.map(sanitizeStringForSwift) : [];

    // レスポンスヘッダーに明示的に charset=utf-8 を指定
    res.setHeader("Content-Type", "application/json; charset=utf-8");

    return res.status(200).json({
      title: sanitizedTitle || "（タイトル未設定）",
